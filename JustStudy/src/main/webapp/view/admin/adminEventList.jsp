    ...
    <div id="main">
        <table cellspacing="0" cellpadding="0" style="border-collapse:collapse">
            ...
            <c:forEach items="${arr_eventDTO}" var="eventDTO">
                <tr>
                    <td>${eventDTO.event_date}</td>
                    <td><a href="AdminEventDetail?event_id=${eventDTO.event_id}">${eventDTO}</a></td>
                    <td>${eventDTO.event_writer}</td>
                    <td>${eventDTO.event_category}</td>
                    <td>${eventDTO.event_type}</td>
                    <td>${eventDTO.event_branch}</td>
                    <c:choose>
                        <c:when test="${eventDTO == 1}">
                            <td>미답변</td>
                        </c:when>
                        <c:otherwise>
                            <td>답변</td>
